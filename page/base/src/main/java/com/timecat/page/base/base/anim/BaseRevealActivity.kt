package com.timecat.page.base.base.anim
 ...
 * @date 2020/5/22
 * @description 将 Activity 的揭露效果写在 Base 类中，需要揭露动画效果时继承
 * @usage
 *
 ...
 */
abstract class BaseRevealActivity : AppCompatActivity(){
    companion object {
    ...
    private fun circularReveal(intent: Intent?){
        ...
        onGlobalLayout = ViewTreeObserver.OnGlobalLayoutListener { //此时既是开始揭露动画的最佳时机
            ...
            mAnimReveal?.addListener(listener)
            mAnimReveal?.start()
        }
    ...
    //这个方法很重要，如果我们应用的启动图标在桌面上的位置有变化，可在此收到新的位置信息，
    // 然而经作者本人实践作用十分有限
